...
public class ConsoleView {
    ...
    private static final int NUMBER_ATTEMPTS_INPUT = 3;
    ...
    public void start(final Player[] players, final Board board) {
        ...
        inputPlayersNames(players);
         ...
         do {
            showCurrentPlayer();
            gameController.move(getCoordinate("x"),getCoordinate("y"));
            showBoard();

         } while ((gameController.winner(board)==null)^(gameController.checkDraw(board)));
    ...
    public int getCoordinate(final String coordinateName) {
        ...
        } while (counter < NUMBER_ATTEMPTS_INPUT);

        return -1;
    }

    private void showCoordinateErrorMessage(){
